...
def dividend_optimize_stream_route():
    ...
    def generate():
        try:
            ...
            dividend_found = 0
            results = []
            yield f"data: {json.dumps({'type': 'meta', 'total_scanned': len(symbols), 'max_results': DIVIDEND_MAX_RESULTS})}\n\n"
            ...
            batches = list(_batched(symbols_to_fetch, DIVIDEND_BATCH_SIZE))
            with ThreadPoolExecutor(max_workers=DIVIDEND_MAX_WORKERS) as executor:
                futures = [executor.submit(_download_batch, batch) for batch in batches]
                for future in as_completed(futures):
                    batch, data = future.result()
            ...
            truncated = False
            if len(display_stocks) > DIVIDEND_MAX_RESULTS:
                display_stocks = display_stocks[:DIVIDEND_MAX_RESULTS]
                truncated = True
